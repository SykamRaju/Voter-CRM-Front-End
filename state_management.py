    ...
    if not states:
        ...
        state_data = {
            ...
            "Status": [state.status for state in states],
             "Edit": ['<a href="#" onClick="open_edit_modal({})">Edit</a>'.format(i) for i in range(len(states))],
            "Delete": ['<a href="#" onClick="open_delete_modal({})">Delete</a>'.format(i) for i in range(len(states))]
        }
        df = pd.DataFrame(state_data)
        st.dataframe(df, unsafe_allow_html=True)

        # for state in states:
        #     st.write(f"- {state.name} (State Code: {state.state_code}) {state.status}")
